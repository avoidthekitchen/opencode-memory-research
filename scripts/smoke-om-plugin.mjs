...
async function runPluginSmoke() {
  ...
  if (!system.system.some((entry) => entry.includes("Before answering, call om_observe exactly once."))) {
    throw new Error("Smoke test failed: no observe-required system block was injected.")
  }
  if (status.current.requiredTool !== "om_observe") {
    throw new Error(`Smoke test failed: expected requiredTool=om_observe, got ${status.current.requiredTool}`)
  }

  await plugin.tool.om_observe.execute(
    {
      observations: "Date: Mar 3, 2026\n* 🔴 (09:00) User asked for a smoke-test observation flow.\n* 🟡 (09:01) Assistant replied with a greeting.",
      currentTask: "Primary: Continue the smoke-test flow.",
      suggestedResponse: "The assistant should continue normally after maintenance.",
  ...
  const postStatusText = await plugin.tool.om_status.execute({}, { sessionID: "smoke-session", metadata() {} })
  const postStatus = JSON.parse(postStatusText)

  if (!postObserveSystem.system.some((entry) => entry.includes("<observations>"))) {
    throw new Error("Smoke test failed: OM block was not injected after om_observe.")
  }
  if (postStatus.current.lastObservedTurnAnchor !== "user-1") {
    throw new Error(`Smoke test failed: expected lastObservedTurnAnchor=user-1, got ${postStatus.current.lastObservedTurnAnchor}`)
  }
  if (postStatus.current.requiredTool) {
    throw new Error(`Smoke test failed: expected no pending maintenance, got ${postStatus.current.requiredTool}`)
  }
...
function assertToolSchemas(plugin) {
  ...
  const expectedObserveArgs = ["confirmObservedThrough", "currentTask", "observations", "suggestedResponse"]
  const expectedReflectArgs = ["compressionLevel", "currentTask", "observations", "suggestedResponse"]

  if (JSON.stringify(observeArgs) !== JSON.stringify(expectedObserveArgs)) {
    throw new Error(`Smoke test failed: unexpected om_observe args ${JSON.stringify(observeArgs)}`)
  }
  if (JSON.stringify(reflectArgs) !== JSON.stringify(expectedReflectArgs)) {
    throw new Error(`Smoke test failed: unexpected om_reflect args ${JSON.stringify(reflectArgs)}`)
  }
}
